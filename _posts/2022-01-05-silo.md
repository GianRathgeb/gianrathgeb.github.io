...
```bash
┌──(user㉿KaliVM)-[/hackthebox/oscp-prep/silo]
└─$ python3 /tools/dirsearch/dirsearch.py -u http://silo.htb/ -e php -x 403,404 -t 50                                                                                                                           1 ⨯
...
12:45:49 WARNING -: Impossible to fetch all the rows of the query select httpuritype('http://0.0.0.0/').getclob() from dual: `ORA-29273: HTTP request failed ORA-06512: at "SYS.UTL_HTTP", line 1819 ORA-24247: network access denied by access control list (ACL) ORA-06512: at "SYS.HTTPURITYPE", line 34`
[-] KO
